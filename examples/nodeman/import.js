...
conn.query("CREATE TABLE IF NOT EXISTS modules(id INTEGER AUTO_INCREMENT, anchor VARCHAR(254), name VARCHAR(254), description TEXT, description_html TEXT, PRIMARY KEY (id), FULLTEXT(name), FULLTEXT(description)) CHARACTER SET 'utf8' COLLATE 'utf8_general_ci'");
...
process.stdio.addListener('close', function (data) {
	...
	conn.query(["INSERT INTO modules(name, anchor, description, description_html) VALUES(?,?,?,?)", section.name, section.anchor, section.description, section.description_html], function(result) {
	    module_id = conn.insert_id;
	    sys.puts(section.name);
	    section.functions.forEach(function(func) {
		sys.puts("  "+func.name);
		conn.query(["INSERT INTO functions(name, anchor, module_id, description, description_html) VALUES(?,?,?,?,?)", func.name, func.anchor, module_id, func.description, func.description_html]);
	    });
