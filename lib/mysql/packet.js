...
var net2value = function(adata, type, unsigned, charset) {
    switch(type.id) {
	...
	return new types.Time(ret[0], ret[1], ret[2], ret[3], ret[4], ret[5]!=0, ret[6]);
...
var value2net = function(v, charset) {
	...
	if(v.constructor.toString().indexOf(" Date(")) {
	    ...
	    val = pack.pack("CvCCCCC", 7, v.getYear(), v.getMonth(), v.getDay(), v.getHours(), v.getMinutes(), v.getSeconds());
	}
	else if(v.year || v.month || v.day || v.hours || v.minutes || v.seconds) {
            type = constants.field.TYPE_DATETIME;
	    val = pack.pack("CvCCCCC", 7, v.year, v.month, v.day, v.hours, v.minutes, v.seconds);
	}
	else {
...
StatementRecordPacket.parse = function(data, fields, charset) {
	...
	i += 1;
        if(null_bit_map.substring(i+2,i+3) == '1') {
            ...
            var v = net2value(adata, f.type, unsigned, charset);
            if(typeof(v)=='number') { // v.is_a? Mysql::Time) { // TODO
		return v;
	    }
	    else if(f.type == constants.field.TYPE_BIT || !!(f.flags & constants.field.BINARY_FLAG)) {
		return v; // Charset.to_binary(v) // TODO
	    }
            else {
		return v; // charset.force_encoding(v) // TODO
	    }
	}
    });
}
