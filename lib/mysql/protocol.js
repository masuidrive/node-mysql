...
Protocol.prototype.query_command = function(query) {
	...
	this.raw_conn.reset();
	this.raw_conn.write(pack.pack("CZ*", constants.com.QUERY, query))
	    .addCallback(scope(this, function(buff) {
		this.get_result()
...
Protocol.prototype.stmt_prepare_command = function(stmt) {
	...
	this.raw_conn.reset();
	this.raw_conn.write(pack.pack("Ca*", constants.com.STMT_PREPARE, stmt)) // TODO: charset.convert(stmt)));
	this.raw_conn.read()
            .addCallback(scope(this, function(buff) {
		...
		var func = scope(this, function() {
			...
			this.raw_conn.read(res_packet.field_count) // skip parameter packet
				...
				this.read_eof_packet().addCallback(scope(this, function() {
