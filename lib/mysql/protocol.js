...
Protocol.prototype.query_command = function(query) {
	...
	this.raw_conn.reset();
	this.raw_conn.write(pack.pack("CZ*", constants.com.QUERY, char_encode(query)))
	    .addCallback(scope(this, function(buff) {
		this.get_result()
...
Protocol.prototype.stmt_prepare_command = function(stmt) {
	...
	this.raw_conn.reset();
	this.raw_conn.write(pack.pack("Ca*", constants.com.STMT_PREPARE, char_encode(stmt)))
	this.raw_conn.read()
            .addCallback(scope(this, function(buff) {
...
Protocol.prototype.stmt_execute_command = function(stmt_id, values) {
    ...
    this.raw_conn.reset();
    this.raw_conn.write(packet.ExecutePacket.serialize(stmt_id, constants.stmt.CURSOR_TYPE_NO_CURSOR, values));
    this.get_result()
        .addCallback(function(nfields) {
	    promise.emitSuccess(nfields);
	})
