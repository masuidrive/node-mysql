...
Protocol.prototype.close = function() {
    if(this.conn.readyState()=='open' && this.state!='INIT') {
	this.wait_ready_state(undefined, scope(this, function(error) {
	    this.conn.close();
	}));
    }
    else {
...
Protocol.prototype.query_command = function(query, immediately) {
    ...
    this.wait_ready_state('PROCESS', scope(this, function(error) {
	if(error) {
	    this.set_state('READY');
...
Protocol.prototype.get_result = function() {
	...
	.addCallback(scope(this, function(buff) {
		...
		fs.readFile(filename, 'UTF-8', scope(this, function(err, data) {
		    if(err) {
			promise.emitError(new errors.ClientError("Can't read '"+filename+"'"));
			this.set_state('READY');
			return;
		    }
		    var error_handler = scope(this, function(error) {
			this.set_state('READY');
			promise.emitError(error);
		    });
