...
Protocol.prototype.authenticate = function(user, passwd, db, flag, charset_str) {
	...
	.addCallback(scope(this, function(buff) {
		...
		this.charset = Charset.by_name("utf8_unicode_ci"); //Charset.by_number(init_packet.server_charset);
	    }
...
Protocol.prototype.query_command = function(query) {
	...
	this.raw_conn.write(pack.pack("CZ*", constants.com.QUERY, this.charset.convertToBytes(query)))
	    .addCallback(scope(this, function(buff) {
		this.get_result()
...
Protocol.prototype.retr_all_records = function(fields, each_callback, end_callback) {
	...
	this.raw_conn.read()
		...
		if(is_eof_packet(data)) {
			...
			rec.push(fields[i].type.convert(val, this));
		    }
		    each_callback(rec);
		    get_line();
		}
...
Protocol.prototype.stmt_prepare_command = function(stmt) {
	...
	this.raw_conn.reset();
	this.raw_conn.write(pack.pack("Ca*", constants.com.STMT_PREPARE, this.charset.convertToBytes(stmt)))
	this.raw_conn.read()
            .addCallback(scope(this, function(buff) {
		var res_packet = packet.PrepareResultPacket.parse(buff);
		var func = scope(this, function() {
		    if(res_packet.field_count > 0) {
			this.raw_conn.read(res_packet.field_count) // skip parameter packet
		...
		if(res_packet.param_count > 0) {
		    this.raw_conn.read(res_packet.param_count) // skip parameter packet
		        .addCallback(scope(this, function(){
			    this.read_eof_packet()
				.addCallback(scope(this, function() {
				    func();
...
Protocol.prototype.stmt_close_command = function(stmt_id) {
	...
	this.raw_conn.reset();
	this.raw_conn.write(pack.pack("CV", constants.com.STMT_CLOSE, stmt_id));
    }
}
...
Protocol.prototype.stmt_execute_command = function(stmt_id, values) {
    ...
    this.raw_conn.reset();
    this.raw_conn.write(packet.ExecutePacket.serialize(stmt_id, constants.stmt.CURSOR_TYPE_NO_CURSOR, values, this.charset));
    this.get_result()
        .addCallback(function(nfields) {
	    promise.emitSuccess(nfields);
	})
...
Protocol.prototype.check_state = function(st, promise) {
	...
	if(promise) promise.emitError("Protocol Error", "Unmatch protocol state "+st+"(expect) != "+this.state);
	return false;
    }
}
