...
var RawConnection = function(port, hostname) {
    ...
    var packet_seq; // packet sequence number
...
/*
  ...
  # === Argument
  # data :: [String] packet data. If data is nil, write empty packet.
*/
RawConnection.prototype.write = function(data) {
	...
	while(data) {
	    ...
	    this.tcp_conn.send(pack.pack("CvC", d.length%256, d.length/256, this.seq), 'binary');
	    this.tcp_conn.send(d, 'binary');
	    this.seq = (this.seq + 1) % 256;
	}
	//Timeout.timeout @write_timeout do
	//  @sock.flush
